...
def TestSampler(df, rng):
    ...
    X_test = df_test.drop(['ID', 'MedalEarned'], axis= 1)
    Y_test = df_test['MedalEarned']
...
def TrainValidate(df, rng):
    ...
    # Reduce and split X and Y dataframes
    X_validate = df_validate.drop(['ID', 'MedalEarned'], axis= 1)
    Y_validate = df_validate['MedalEarned']
    X_train =    df_train.drop(['ID', 'MedalEarned'], axis= 1)
...
# * Run model
def RunModel(df, rng, cop, iterations, l_rate):
    # Make X_train, Y_train, X_validate, Y_validate
    X_train, Y_train, X_validate, Y_validate = TrainValidate(df, rng)
...
def RunMore(df, rng, cop, times, iterations, l_rate):
    W_list = []
    B_list = []
    val_acc_list = []
    val_occ_list = []
    ...
    # Create test sample
    df_testless, X_test, Y_test = TestSampler(df, rng)
    
    for i in range(times):
        # Run model
        W, B, val_acc, val_occ_dic = RunModel(df_testless, rng, cop, iterations, l_rate)
...
def Decathlon(df, W_array, B_array, cop):
    dec_acc_list = []
    dec_occ_list = []
    
    # Reduce and split X and Y dataframes
    X_dec = df.drop(['MedalEarned'], axis= 1)
    Y_dec = df['ID', 'MedalEarned']
...
# ! Run the model
if True:
    filepath = 'Datasets/dec_sep_NOCdummy.csv'
    ...
    RunMore(df, rng, cop = 0.65, times= 50, iterations= 5000, l_rate= 0.02)
